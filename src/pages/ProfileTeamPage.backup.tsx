...
export default function ProfileTeamPage() {
  ...
  const [workspaceBranding, setWorkspaceBranding] = useState('Modern Logistics Hub');
  const [showCustomizeModal, setShowCustomizeModal] = useState(false);
  const [showMemberRoleModal, setShowMemberRoleModal] = useState(false);
  const [selectedMember, setSelectedMember] = useState<any>(null);
  const [apiKey, setApiKey] = useState('sk_live_exportrack_' + Math.random().toString(36).substr(2, 20).toUpperCase());
  ...
  // NEW: Handle role change for selected member
