...
export default function UploadDocumentsPage() {
  ...
  const sortedDocs = useMemo(
    () => [...(shipment?.documents ?? [])].sort((a, b) => b.uploadedAt.localeCompare(a.uploadedAt)),
    [shipment?.documents]
  );
  ...
  const handleUpload = () => {
    ...
    files.forEach((file) => {
      addDocument(shipment.id, {
        type: documentType,
        fileName: file.name,
        fileFormat: detectFormat(file.name),
        uploadedBy: user?.name ?? 'Coordinator'
      });
    });
  ...
  return (
    <div>
      <PageHeader
        ...
        subtitle="Supported formats: PDF, JPG, PNG. Mobile camera scan is enabled with capture mode."
        action={
          <Link
      ...
      <section className="grid gap-6 lg:grid-cols-[1.2fr_1fr]">
        <article className="rounded-2xl border border-slate-200 bg-white p-5 shadow-soft">
          ...
          <div className="mt-4 grid gap-4 md:grid-cols-2">
            <div>
              ...
              <select
                id="doc-type"
                value={documentType}
                onChange={(event) => setDocumentType(event.target.value as UploadDocumentInput['type'])}
            ...
            <div>
              <label htmlFor="doc-files" className="mb-1 block text-sm font-medium text-slate-700">
                Files (with camera capture support)
              </label>
              <input
                id="doc-files"
                type="file"
          ...
          <div className="mt-4 rounded-xl border border-dashed border-teal-300 bg-teal-50 p-3 text-sm text-teal-800">
            <p className="font-semibold">Selected files ({files.length})</p>
            <ul className="mt-2 space-y-1 text-xs text-teal-900">
              {files.length ? files.map((file) => <li key={file.name}>{file.name}</li>) : <li>No files selected yet.</li>}
            </ul>
          </div>

          <button type="button" onClick={handleUpload} className="mt-4 rounded-xl bg-navy-700 px-4 py-2 text-sm font-semibold text-white hover:bg-navy-800">
        ...
        <article className="rounded-2xl border border-slate-200 bg-white p-5 shadow-soft">
          ...
          <div className="space-y-3">
            <button
              type="button"
              onClick={() => window.alert(`Downloading all files for ${shipment.id} (mock).`)}
            ...
            <Link
              ...
              className="block rounded-lg border border-slate-300 px-3 py-2 text-center text-sm font-semibold text-slate-700 hover:bg-slate-100"
            >
