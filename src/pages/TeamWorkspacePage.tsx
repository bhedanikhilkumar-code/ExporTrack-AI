        ...
        <div className="flex flex-wrap gap-1.5">
          {ALL_PERMISSIONS.map((perm: any) => {
            ...
            return (
              <span
                key={perm}
                className={`px-2 py-1 rounded-md text-[9px] font-bold ${has
  ...
  const handleInvite = async () => {
    ...
    try {
      ...
      const result = await sendInvite({
        ...
        inviterName: state.user?.name || 'Team Admin',
      ...
      try {
        ...
        alert(fallbackErr.message || 'Failed to send invitation');
      }
    } finally {
          ...
          <div className="flex p-1 rounded-xl bg-slate-100/50 dark:bg-slate-800/50 border border-slate-200/50 dark:border-slate-800/50 mb-6 w-full max-w-lg">
            {(['members', 'invites', 'roles', 'activity'] as const).map(tab => (
              <button
                ...
                className={`flex-1 py-2 px-3 rounded-lg text-xs font-black uppercase tracking-widest transition-all ${activeTab === tab
              ...
              <div className="space-y-4">
                {[
                  ...
                  <div key={idx} className="relative flex gap-4">
                    ...
                    <div className={`mt-1 flex h-8 w-8 shrink-0 items-center justify-center rounded-full border border-white text-white shadow-sm dark:border-slate-800 ${entry.type === 'role' ? 'bg-rose-500' :
                    ...
                    {WORKSPACE_ROLES.map(role => {
                      ...
                      return (
                        <button
                          ...
                          className={`px-3 py-2.5 rounded-xl text-xs font-bold transition-all border ${inviteRole === role
